...
class DB:
    ...
    def get_avist_per_day(self):
        sql_query = """
        SELECT COUNT(1) AS entries, DATE(dia_hora) as date
        FROM detalle_avistamiento
        GROUP BY DATE(dia_hora)
        ORDER BY date
        LIMIT 0 , 30"""
        self.cursor.execute(sql_query)
        return self.cursor.fetchall()
    ...
    def get_state_month_count(self):
        sql_query = """
        SELECT COUNT(*) as conteo,estado, MONTH(dia_hora) as mes
        FROM detalle_avistamiento
        GROUP BY estado,mes
        ORDER BY mes"""
        self.cursor.execute(sql_query)
    ...
    def get_map_data(self):
        sql_query = """
        SELECT DA.id, DA.dia_hora, DA.tipo, DA.estado, DA.avistamiento_id, FT.ruta_archivo, FT.nombre_archivo, CO.nombre
        FROM detalle_avistamiento DA, foto FT, avistamiento AV, comuna CO
        WHERE FT.detalle_avistamiento_id = DA.id AND AV.comuna_id = CO.id AND DA.avistamiento_id = AV.id
        ORDER BY CO.nombre, DA.id"""
        self.cursor.execute(sql_query)
        return self.cursor.fetchall()