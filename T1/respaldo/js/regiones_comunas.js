...
function make_regiones() {
    node = document.getElementById("regiones");
...
function region_change(node) {
    comunas_node = node.parentNode.getElementsByClassName("comunas")[0];
    options = "";
    region_actual = node.value;
    for (comuna in comunas[region_actual]) {
        options += `<option>${comunas[region_actual][comuna]}</option>`;
      }
